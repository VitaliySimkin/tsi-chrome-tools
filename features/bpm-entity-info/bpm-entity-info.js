window.NSEntityInfo = {
	...
	"columnsProp": [
		...
		{
			"name": "lookup",
			"caption": "LookupSchemaName",
			getter(item) {
				return item.referenceSchemaName || "";
			}
		}
	],
	...
	handleDocumentClick() {
		...
		const onClick = function() {
			const clickedInsidePage = Boolean(event.target.closest("#ns-entity-info-cnt"));
	...
	generateColumnInfoTable(entity) {
		const cnt = this.createElement("table", { "className": "ns-ei-column-table" });
	...
	generateColumnInfoContent(entity) {
		const content = this.createElement("tbody", { "className": "ns-ei-column-table-content" });
		const columns = Object.values(entity.columns);
		const appenColumnInfoToContent = column => content.appendChild(this.createColumnInfoRow(column));
	...
	createColumnInfoRow(column) {
		const row = this.createElement("tr", { "className": "ns-ei-column-info-row" });
		row.setAttribute("column-name", column.name);
		this.columnsProp.forEach(columnProp => {
			const el = this.createElement("td");
	...
	generateColumnInfoHeader() {
		const header = this.createElement("thead", { "className": "ns-ei-column-table-header" });
		const captionRow = this.createElement("tr");
		header.appendChild(captionRow);
		this.columnsProp.forEach(column => {
			const el = this.createElement("th");
	...
	getInfoNotFound() {
		const cnt = this.createElement("span", { "className": "ns-ei-info-not-found" });
	...
	generateHeader() {
		const header = this.createElement("div", { 'className': "ns-ei-header" });
	...
	createHeaderFilter(header, caption, onEnter, scope) {
		const cnt = this.createElement("span");
		const text = this.createElement("span", { "className": "ns-ei-filter-caption" });
	...
	filterColumns() {
		const value = this.columnFilter.value.toLowerCase();
		const filter = col => col.name.toLowerCase().includes(value) || col.caption.toLowerCase().includes(value);
		if (!value) {
			filter = () => true;
		}
		const columns = Object.values(this.entity.columns);
		const visibleColumns = columns.filter(filter).map(col => col.name);
		this.tbody.childNodes.forEach(item => {
			const itemColumn = item.getAttribute("column-name");
			const visible = visibleColumns.includes(itemColumn);
